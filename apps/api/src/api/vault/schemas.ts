...
export const syncBody = {
  type: 'object',
  required: ['category', 'payload', 'signature', 'ts'],
  additionalProperties: false,
  properties: {
    category,
    /** P2 = XChaCha20(Ki, P1), chiffré côté client. Le serveur ne le lit pas. */
    payload: { ...base64, maxLength: 96 * 1024 * 1024 },
    /** Ed25519.sign(SHA256("relais:vault:v1|catégorie|ts|" ‖ payload), ed25519_sk) — 64 bytes (audit MEDIUM-7). */
    signature: { ...base64, maxLength: 128 },
    /** Horodatage client en ms : fenêtre ± 5 min, strictement croissant par catégorie — ni rejeu ni retour en arrière. */
