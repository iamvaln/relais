...
//
// - Le secret base32 est stocké scellé (lib/enc.ts : `enc1:`, AES-256-GCM,
//   clé = SHA256(TOTP_ENC_KEY)). Une valeur legacy sans préfixe
...
import { isSealed, openSealed, seal } from './enc.js'
...
export function encryptTotpSecret(plain: string): string {
  return seal(plain, env().TOTP_ENC_KEY)
}

export function decryptTotpSecret(stored: string): string {
  if (!isSealed(stored)) return stored // legacy en clair
  return openSealed(stored, env().TOTP_ENC_KEY)
}
