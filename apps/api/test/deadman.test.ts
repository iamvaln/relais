...
import { closeAll, lastEmailTo, mailbox, pushbox, resetState } from './helpers.js'
...
async function subscribed(userId: string) {
  await prisma().push_tokens.create({ data: { user_id: userId, token: `sub_${userId}`, platform: 'android' } })
}
...
describe('sweep — relances J+7 / J+14 / J+21', () => {
  it('ne fait rien pour une échéance à venir ni pour un retard de moins de 7 jours', async () => {
    ...
    mailbox.clear()
    expect(await sweep(NOW)).toEqual({ relances: 0, triggered: 0, pushes: 0, resumed: 0 })
  ...
  it('J+7 : relance 1 par email, tracée dans checkin_relances et email_log, une seule fois', async () => {
    const o = await makeOwner()
    await activateTransmission(o)
    await subscribed(o.userId)
    mailbox.clear()
    pushbox.clear()
    await overdueBy(o.userId, 7)
    expect(await sweep(NOW)).toEqual({ relances: 1, triggered: 0, pushes: 1, resumed: 0 })

    const mail = lastEmailTo(o.email)
    expect(mail?.subject).toBe('Un petit signe ?')
    // E4-US02 : la relance 1 part aussi en push (lot 5 mobile)
    expect(pushbox.sent.map((p) => p.title)).toEqual(['On pense à toi'])
    ...
    expect(await sweep(new Date(NOW.getTime() + DAY))).toEqual({ relances: 0, triggered: 0, pushes: 0, resumed: 0 })
  ...
  it('J+14 puis J+21 : relances 2 et 3, avec les templates correspondants', async () => {
    ...
    expect(await sweep(NOW)).toEqual({ relances: 1, triggered: 0, pushes: 0, resumed: 0 })
...
describe('sweep — déclenchement après le silence configuré', () => {
  ...
  it('silence écoulé (1 mois) et trois relances : statut triggered', async () => {
    ...
    expect(await sweep(NOW)).toEqual({ relances: 0, triggered: 1, pushes: 0, resumed: 0 })
...
describe('runDeadman — le job quotidien enchaîne balayage et ouverture', () => {
  it('silence écoulé → triggered → transmission ouverte et contacts prévenus dans le même passage', async () => {
    ...
    await overdueBy(o.userId, 31, { relance_count: 3, last_relance_at: new Date(NOW.getTime() - 10 * DAY) })
    mailbox.clear()
    expect(await runDeadman(NOW)).toEqual({ relances: 0, triggered: 1, pushes: 0, resumed: 0, transmissions: 1, contacts_notified: 2 })
...
describe('sweep — pushs du jour (lot 5 mobile, décision du 12/09/2026)', () => {
  it('le jour de l’échéance : un push « Un petit signe ? », aucun email, aucune relance, et rien de plus le lendemain', async () => {
    const o = await makeOwner()
    await activateTransmission(o)
    await subscribed(o.userId)
    mailbox.clear()
    pushbox.clear()
    await overdueBy(o.userId, 0.5)
    expect(await sweep(NOW)).toEqual({ relances: 0, triggered: 0, pushes: 1, resumed: 0 })
    expect(pushbox.sent).toHaveLength(1)
    expect(pushbox.sent[0]).toMatchObject({ title: 'Un petit signe ?', route: '/checkin' })
    expect(mailbox.sent).toHaveLength(0)
    expect((await config(o.userId)).relance_count).toBe(0)

    expect(await sweep(new Date(NOW.getTime() + DAY))).toEqual({ relances: 0, triggered: 0, pushes: 0, resumed: 0 })
    expect(pushbox.sent).toHaveLength(1)
  })

  it('sans abonnement push, rien ne part et le compteur reste à zéro', async () => {
    const o = await makeOwner()
    await activateTransmission(o)
    pushbox.clear()
    await overdueBy(o.userId, 0.5)
...
describe('sweep — pause (E4-US04, E6-US04)', () => {
  async function pausedUntil(userId: string, days: number) {
    await prisma().transmission_configs.update({
      where: { user_id: userId },
      data: { status: 'paused', paused_at: NOW, pause_until: new Date(NOW.getTime() + days * DAY) },
    })
  }

  it('J-3 avant la fin : un rappel par email (pause_ending) et par push, une seule fois', async () => {
    const o = await makeOwner()
    await activateTransmission(o)
    await subscribed(o.userId)
    await pausedUntil(o.userId, 3)
    mailbox.clear()
    pushbox.clear()
    expect(await sweep(NOW)).toEqual({ relances: 0, triggered: 0, pushes: 1, resumed: 0 })
    const mail = lastEmailTo(o.email)
    expect(mail?.subject).toContain('pause')
    expect(pushbox.sent[0]).toMatchObject({ title: 'Ta pause se termine bientôt', route: '/transmission' })
    const logs = await prisma().email_log.findMany({ where: { user_id: o.userId, email_type: 'pause_ending' } })
    expect(logs).toHaveLength(1)
    expect((await config(o.userId)).status).toBe('paused')

    // J-2, J-1 : plus rien
    expect(await sweep(new Date(NOW.getTime() + DAY))).toEqual({ relances: 0, triggered: 0, pushes: 0, resumed: 0 })
    expect(await sweep(new Date(NOW.getTime() + 2 * DAY))).toEqual({ relances: 0, triggered: 0, pushes: 0, resumed: 0 })
    expect(mailbox.sent).toHaveLength(1)
  })

  it('fin de pause atteinte : reprise automatique, check-in replanifié à + fréquence, relances remises à zéro', async () => {
    const o = await makeOwner()
    await activateTransmission(o, { frequency: 2 })
    await pausedUntil(o.userId, -0.5)
    await prisma().transmission_configs.update({ where: { user_id: o.userId }, data: { relance_count: 2 } })
    mailbox.clear()
    expect(await sweep(NOW)).toEqual({ relances: 0, triggered: 0, pushes: 0, resumed: 1 })
    const cfg = await config(o.userId)
    expect(cfg.status).toBe('active')
    expect(cfg.pause_until).toBeNull()
    expect(cfg.paused_at).toBeNull()
    expect(cfg.relance_count).toBe(0)
    expect(cfg.next_checkin_due?.toISOString()).toBe(new Date(NOW.getTime() + 14 * DAY).toISOString())
    expect(mailbox.sent).toHaveLength(0)
    // Une pause reprise ne l'est qu'une fois
    expect(await sweep(NOW)).toEqual({ relances: 0, triggered: 0, pushes: 0, resumed: 0 })
  })
})
