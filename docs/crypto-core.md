...
| `relay` | Côté contact : réponses → K_i → vérification locale → parts déchiffrées ; puis N parts → Kj → P2 ouvert (les fiches chiffrées du coffre, que l'app déchiffre une à une) ; `openSecret` | Techniques §5.6, DEC-13 |
