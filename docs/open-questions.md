...
| Audit de sécurité interne (11/09/2026) | 15 constats sur `apps/api`. Corrigés : pause qui ne reprenait pas (lot 5), purge par un seul contact, annulation admin bloquante, limites de débit contournables et force brute TOTP (PR audit HIGH). Restent : détournement d'une inscription en cours, oracles d'énumération d'emails, signature du vault non liée à la catégorie, gardes de configuration en production, suppression RGPD des OTP d'inscription, ouverture non résiliente si une sealed box ne s'ouvre plus, injection CSV, rôle < N porteurs accepté côté serveur, `POST /auth/keys` sans step-up, TOTP rejouable dans sa fenêtre, codes d'erreur trompeurs. |
